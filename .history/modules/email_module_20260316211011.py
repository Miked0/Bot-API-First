...
class EmailService:
    ...
    def send_email(
        ...
        try:
            ...
            logger.error(f"Erro SMTP ao enviar e-mail para {destinatario}: {type(e).__name__}", exc_info=False)
            return False
    ...
    def _get_subject(template_name: str, nome: str) -> str:
        ...
        subjects = {
            "boas_vindas": f"Bem-vindo(a) à Eztec, {nome}!",
            "manual_proprietario": f"Manual do Proprietário - {nome}",
            "administradora_iptu": f"Informações Administradora e IPTU - {nome}",
        }

        return subjects.get(
            template_name,
            f"Eztec - Comunicado para {nome}"
        )
