...
def process_cliente(
    ...
    if status_cand == 'APROVADO':
        # 1) E-mail de aprovação (boas_vindas)
        # 2) E-mail com próximos passos (manual_proprietario)
        ...
        # E-mail de reprovação (administradora_iptu)
        templates = ['administradora_iptu']
        logger.info(f"   📋 Candidato REPROVADO: {len(templates)} e-mail programado")
    else:
        # EM_ANALISE ou outro status: sem e-mails automáticos por enquanto
        templates = []
